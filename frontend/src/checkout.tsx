...
export function CheckoutView({ cart, session, onCompleted }: { cart: Cart; session: Session | null; onCompleted: (id: string, account?: Session) => void }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    }).then((result) => onCompleted(result.orderId, result.account)).catch((reason: Error) => setMessage(reason.message));
  };
  ...
  return <section className="mx-auto max-w-3xl px-6 py-10">
    ...
    <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <fieldset className="mt-6"><legend className="text-sm font-semibold">Método de pago</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">
      ...
      <button disabled={cart.items.length === 0} className="mt-6 w-full rounded-lg bg-slate-900 px-4 py-3 font-semibold text-white disabled:bg-slate-300">Confirmar pedido</button>
    </form>
  </section>;
}
