...
export function CatalogPage({ onOpenProduct }: { onOpenProduct: (id: string) => void }) {
  ...
  return <section className="mx-auto max-w-6xl px-6 py-10">
    ...
    {error ? <div className="rounded-xl border border-red-200 bg-red-50 p-8 text-center text-red-700">{error}. Verifica que el backend esté iniciado.</div> : products.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-600">No encontramos productos para esta búsqueda.</div> : <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{products.map((product) => {
      ...
      return <article key={product.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex h-44 items-end bg-gradient-to-br from-amber-100 via-slate-100 to-sky-200 p-5"><span className="text-5xl">⚽</span></div><div className="space-y-4 p-5"><div className="flex items-center justify-between gap-2"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-semibold uppercase text-amber-800">{product.category}</span><span className={`text-xs font-semibold ${available ? 'text-emerald-600' : 'text-red-600'}`}>{available ? (isMadeToOrder ? 'Disponible sobre pedido' : 'Disponible') : 'Agotada'}</span></div><div><h2 className="text-xl font-bold">{product.name}</h2><p className="mt-2 text-sm text-slate-600">{product.description}</p></div><div className="flex items-center justify-between"><span className="text-xl font-bold">${firstVariant?.price.toFixed(2) ?? '0.00'}</span><span className="text-xs text-slate-500">{isMadeToOrder ? 'Sobre pedido' : 'Entrega inmediata'}</span></div><div className="flex items-center justify-between gap-2"><span className="text-sm text-slate-600">{isMadeToOrder ? `${activeVariants.length} opciones` : `Talla: ${sizes.join(', ') || 'Única'}`}</span><button type="button" onClick={() => onOpenProduct(product.id)} className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white">Ver detalle</button></div></div></article>;
  })}</div>}
  </section>;
}
